...
- whole files are S3 object versions at their original keys;
...
- Delete creates a S3 delete marker and commits its exact `VersionId`.
...
Large files use provider-managed multipart upload. Prolly does not store parts
...
GC walks commits, node packs, logical versions, and exact physical bindings. It
