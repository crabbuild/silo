...
use crate::{
    ...
    GetRequest, IdempotencyRetention, ImmutablePut, IndexedOperation, JournalIndexRebuildChunk,
    JournalIndexRebuildChunkId, MutableControlStore, ObjectPath, ObjectPlane, OperationId,
    OperationIndexHead, OperationIndexSegment, OperationIndexSegmentId, OperationIndexSegmentRef,
    PublicationEventId, RefGeneration, RepositoryId, Result, RetryAdvice, ShardedBranchPublisher,
...
pub struct OperationIndexAdvanceReport {
...
pub struct OperationIndexRebuildCursor {
...
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct OperationIndexRebuildStep {
    pub cursor: OperationIndexRebuildCursor,
    pub indexed_events: usize,
    pub segments_written: usize,
    pub complete: bool,
}
...
#[derive(Clone)]
pub struct SegmentedOperationIndex<P: ObjectPlane> {
    plane: Arc<P>,
...
struct LoadedHead {
    value: OperationIndexHead,
    token: StorageToken,
}

impl<P: ObjectPlane> SegmentedOperationIndex<P> {
    pub fn new(
        plane: Arc<P>,
    ...
    pub fn new_with_limits(
            ...
            || max_unindexed_events > 1_000_000
        {
            return Err(Error::new(
                ...
                "operation-index segment limits are invalid",
            ));
        }
    ...
    pub async fn advance(
        ...
        branch: &str,
        now_millis: u64,
    ) -> Result<OperationIndexAdvanceReport> {
        ...
        {
            return Ok(OperationIndexAdvanceReport {
        ...
        Ok(OperationIndexAdvanceReport {
    ...
    pub async fn start_rebuild(
        ...
        branch: &str,
        job: OperationId,
        snapshot: PublicationEventId,
        snapshot_generation: RefGeneration,
        oldest_chunk: JournalIndexRebuildChunkId,
        ...
        Ok(OperationIndexRebuildCursor {
            repository: self.repository,
            branch: branch.to_string(),
            job,
            snapshot,
    ...
    pub async fn advance_rebuild(
        ...
        expected_chunk: JournalIndexRebuildChunkId,
        max_events: usize,
        now_millis: u64,
    ...
    async fn push_segment(
        &self,
        branch: &str,
        levels: &mut Vec<Vec<OperationIndexSegmentRef>>,
        reference: OperationIndexSegmentRef,
    ...
    async fn store_segment(
        ...
        current_generation: RefGeneration,
        now_millis: u64,
    ) -> Result<Option<OperationIndexSegmentRef>> {
            ...
            match unique.entry(entry.operation) {
                ...
                std::collections::btree_map::Entry::Occupied(_) => {
                    return Err(Error::new(
                        ...
                        "one operation ID published different journal events",
                    ));
                }
            }
        }
        ...
        let segment = OperationIndexSegment {
            repository: self.repository,
            branch: branch.to_string(),
            level,
        ...
        let max_created_at_millis = segment
            ...
            .max()
            .expect("nonempty segment");
        Ok(Some(OperationIndexSegmentRef {
            id,
            level,
            min_generation,
            max_generation,
    ...
    async fn load_segment(
        ...
        let segment: OperationIndexSegment = decode_canonical(&stored.bytes)?;
        segment.validate()?;
    ...
    fn validate_rebuild_cursor(&self, cursor: &OperationIndexRebuildCursor) -> Result<()> {
    ...
    async fn load_head(&self, branch: &str) -> Result<Option<LoadedHead>> {
        ...
        let value: OperationIndexHead = decode_canonical(&stored.bytes)?;
    ...
    fn validate_head(&self, head: &OperationIndexHead) -> Result<()> {
    ...
    fn prune_catalog(
        &self,
        levels: &mut [Vec<OperationIndexSegmentRef>],
        current_generation: RefGeneration,
        now_millis: u64,
    ) {
        for level in levels {
    ...
    fn head_path(&self, branch: &str) -> Result<ObjectPath> {
        ...
        ObjectPath::new(format!(
            "{}/operation-index/heads/{}.cbor",
            self.prefix,
            hex::encode(branch.as_bytes())
        ))
    }

    fn segment_path(&self, branch: &str, id: OperationIndexSegmentId) -> Result<ObjectPath> {
        crate::repository::validate_branch(branch)?;
        let encoded = hex::encode(id.as_bytes());
        ObjectPath::new(format!(
            "{}/operation-index/segments/{}/sha256/{}/{}/{}",
            self.prefix,
            hex::encode(branch.as_bytes()),
            &encoded[..2],
            &encoded[2..4],
