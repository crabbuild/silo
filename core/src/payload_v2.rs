...
#[derive(Clone)]
pub struct ImmutablePayloadStoreV2<P: ObjectPlane> {
    plane: Arc<P>,
    prefix: String,
    repository: RepositoryId,
}

impl<P: ObjectPlane> ImmutablePayloadStoreV2<P> {
    ...
    pub async fn put(&self, bytes: Vec<u8>) -> Result<PhysicalObjectBindingV2> {
    ...
    pub async fn get(&self, binding: &PhysicalObjectBindingV2) -> Result<Vec<u8>> {
        binding.validate()?;
        let expected_path = self.path(binding.checksum_sha256)?;
        if binding.path != expected_path {
            return Err(crate::Error::new(
                crate::ErrorCode::CorruptContent,
                "v2 payload binding path does not match its content checksum",
            ));
        }
        ...
        let stored = self
            ...
            .await?
            .ok_or_else(|| {
                crate::Error::new(crate::ErrorCode::MissingClosure, "v2 payload is missing")
            })?;
        if sha256(&stored.bytes) != binding.checksum_sha256 {
            return Err(crate::Error::new(
                crate::ErrorCode::CorruptContent,
                "v2 immutable payload checksum mismatch",
            ));
        }
        Ok(stored.bytes)
    }
