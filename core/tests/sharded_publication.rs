...
use prolly_s3_core::{
    ...
    RefGeneration, RepositoryId, RootManifest, ShardWriterAuthority, ShardedBranchPublisher,
    TakeoverRequest, TreeFormatDigest,
};
...
fn scope(branch: &str) -> AuthorityScope {
...
async fn separate_writers_publish_independent_branch_shards_concurrently() {
    ...
    let publisher_a =
        ShardedBranchPublisher::new(plane.clone(), ".prolly", repository, authority_a.clone())
            .unwrap();
    let publisher_b =
        ShardedBranchPublisher::new(plane.clone(), ".prolly", repository, authority_b.clone())
            .unwrap();
    ...
    let (main, ingest) = tokio::join!(
        publisher_a.create(CommitPublication {
            permit: &main_permit,
            branch: "main",
            commit: &main_root,
            node_pack: None,
        ...
        publisher_b.create(CommitPublication {
...
async fn branch_barrier_fences_the_old_writer_and_activates_the_new_writer() {
    ...
    let old_publisher =
        ShardedBranchPublisher::new(plane.clone(), ".prolly", repository, old_authority.clone())
            .unwrap();
    let new_publisher =
        ShardedBranchPublisher::new(plane, ".prolly", repository, new_authority.clone()).unwrap();
...
#[tokio::test]
async fn publication_stores_real_prolly_nodes_in_the_commit_envelope() {
    let plane = Arc::new(MemoryObjectPlane::new(true));
    ...
    let stored = plane
        ...
        .unwrap()
        .unwrap();
    let envelope = CommitObject::decode_object(&stored.bytes).unwrap();
...
#[tokio::test]
async fn lease_and_ref_updates_remain_within_the_same_control_version_bound() {
    let plane = Arc::new(MemoryObjectPlane::new(true));
    let repository = RepositoryId::from_hash([0xbb; 32]);
    let authority = Arc::new(
        ShardWriterAuthority::new_with_control_retention(
            plane.clone(),
            ".prolly",
            repository,
            Duration::from_secs(60),
    ...
    let authority_path = ObjectPath::new(".prolly/authority/branches/6d61696e/lease.cbor").unwrap();
    ...
    .unwrap();
    let alternate_publisher = ShardedBranchPublisher::new_with_control_retention(
        plane.clone(),
        ".prolly",
        repository,
        authority,
    ...
    let ref_path = ObjectPath::new(".prolly/refs/heads/6d61696e").unwrap();
...
async fn publication_journal_pages_a_stable_branch_snapshot_without_listing() {
    ...
    let publisher =
        ShardedBranchPublisher::new(plane, ".prolly", repository, authority.clone()).unwrap();
    let permit = authority
